...
view: ga_sessions {
  ...
  sql_table_name: `ga-cross-profile-settings.149006597.ga_sessions_*` ;;
...
#   }
  ...
  dimension: custom_dimension_63 {
#     group_label: "User Information"
#     description: "APAC, Central America, EMEA, North America, South America"
#     label: "Region"
  sql: (SELECT value FROM UNNEST(${TABLE}.customdimensions) WHERE index = 63) ;;
}

dimension: custom_dimension_66 {
#     group_label: "User Information"
#     description: "APAC, Central America, EMEA, North America, South America"
#     label: "Region"
sql: (SELECT value FROM UNNEST(${TABLE}.customdimensions) WHERE index = 66) ;;
}
...
view: hits_social {
  ...
  dimension: socialInteractionNetwork {hidden: no}

  dimension: socialInteractionAction {hidden: no}

  dimension: socialInteractions {hidden: no}

  dimension: socialInteractionTarget {hidden: no}

  #dimension: socialNetwork {hidden: yes}

  dimension: uniqueSocialInteractions {hidden: no}

  #dimension: hasSocialSourceReferral {hidden: yes}

  dimension: socialInteractionNetworkAction {hidden: no}
}
